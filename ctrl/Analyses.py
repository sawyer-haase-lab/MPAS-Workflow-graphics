...
class CategoryBinMethodBase(AnalysisBase):
    ...
    def subplotArrangement(self, binValsMap):
        ...
        if len(binValsMap) > 1:
            ...
            nxplots = np.int64(np.ceil(np.sqrt(nsubplots)))
            while nsubplots%nxplots > 0 and nsubplots%nxplots / nxplots <= 0.5: nxplots += 1
            nyplots = np.int64(np.ceil(np.true_divide(nsubplots, nxplots)))
...
class BinValAxisProfile(MultiDimBinMethodBase):
    ...
    def innerloops(self,
        ...
        if self.nFC > 1:
            ...
            nxplots = np.nanmax([np.int64(np.ceil(np.sqrt(nsubplots))), 1])
            while nsubplots%nxplots > 0 and nsubplots%nxplots / nxplots <= 0.5: nxplots += 1
            nyplots = np.int64(np.ceil(np.true_divide(nsubplots, nxplots)))
            ...
            for (varName, varLabel) in self.varMap:
              ...
              for fcTDelta in self.fcTDeltas:
                ...
                for expName in self.expNames:
                  ...
                  expBinMethodLabelsValues += [expName+'--'+b for b in binMethodLabelsValues]
                  expBinMethodLabelsWithMetrics += [expName+'--'+b for b in binMethodLabelsWithMetrics]
